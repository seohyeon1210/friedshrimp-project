document.addEventListener("DOMContentLoaded", function () {
    ...
    const closePasswordFindModalBtn = document.getElementById(
        "closePasswordFindModal"
    );

    // 비밀번호 변경 Modal 요소
    const passwordChangeModal = document.getElementById("passwordChangeModal");
    const closePasswordChangeModalBtn = document.getElementById(
        "closePasswordChangeModal"
    );
    const confirmPasswordFindBtn = document.querySelector(
        ".confirmPasswordFind"
    );
    const savePasswordChangeBtn = document.getElementById(
